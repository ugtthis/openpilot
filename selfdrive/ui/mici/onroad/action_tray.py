...
class DAC3ActionButton(TrayActionButton):
  def __init__(self, click_callback, is_dac3_active):
    super().__init__(click_callback)
  ...
  def _render(self, _) -> None:
    ...
    tint = rl.Color(220, 220, 220, 255) if self.is_pressed else rl.WHITE
    rl.draw_texture_ex(bg, rl.Vector2(rect.x, rect.y), 0.0, rect.width / bg.width, rl.WHITE)
    icon = self._onroad_icon if self._is_dac3_active() else self._dac3_icon
    x = rect.x + (rect.width - icon.width) / 2
    y = rect.y + (rect.height - icon.height) / 2
    rl.draw_texture_ex(icon, rl.Vector2(x, y), 0.0, 1.0, tint)
...
class SidePanelActionTray(Widget):
  def __init__(self, bookmark_callback, settings_callback, dac_callback, dac2_callback, dac3_callback, is_secondary_active, is_dac_active, is_dac2_active, is_dac3_active):
    super().__init__()
    ...
    self._dac3_button = self._child(DAC3ActionButton(self._on_dac3_clicked, is_dac3_active))
    self._bookmark_callback = bookmark_callback
    self._settings_callback = settings_callback
    self._dac_callback = dac_callback
    self._dac2_callback = dac2_callback
    self._dac3_callback = dac3_callback

    for button in (self._bookmark_button, self._dac_button, self._dac2_button, self._dac3_button):
  ...
  def _tray_positions(self, progress: float) -> tuple[float, float, float, float, float]:
    y = self.rect.y + self.rect.height * CENTER_Y_FRAC - BUTTON_SIZE / 2
    visible_bookmark_x = self.rect.x + self.rect.width - (BUTTON_SIZE * 4 + BUTTON_GAP * 3 + RIGHT_MARGIN)
    visible_dac_x = visible_bookmark_x + BUTTON_SIZE + BUTTON_GAP
    visible_dac2_x = visible_dac_x + BUTTON_SIZE + BUTTON_GAP
    visible_dac3_x = visible_dac2_x + BUTTON_SIZE + BUTTON_GAP
    ...
    hidden_dac3_x = hidden_dac2_x + BUTTON_SIZE + BUTTON_GAP
    ...
    if progress <= 1.0:
      ...
      dac3_x = hidden_dac3_x + (visible_dac3_x - hidden_dac3_x) * t
    else:
      ...
      dac3_x = visible_dac3_x - overswipe_distance

    return y, bookmark_x, dac_x, dac2_x, dac3_x
  ...
  def _on_dac3_clicked(self) -> None:
    self._touch_handled_by_tray = True
    self.collapse()
    self._dac3_callback()
  ...
  def _layout(self) -> None:
    y, bookmark_x, dac_x, dac2_x, dac3_x = self._tray_positions(self.open_progress)
    ...
    self._dac3_button.set_rect(rl.Rectangle(dac3_x, y, BUTTON_SIZE, BUTTON_SIZE))
    ...
    if ev.left_pressed:
      if self._state == TrayState.EXPANDED:
        if not (
          ...
          rl.check_collision_point_rec(ev.pos, self._dac2_button.rect) or
          rl.check_collision_point_rec(ev.pos, self._dac3_button.rect)
        ):
  ...
  def _render(self, _) -> None:
    ...
    self._dac3_button.render(self._dac3_button.rect)
