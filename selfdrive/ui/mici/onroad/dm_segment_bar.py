...
def _prompt_threshold(is_active_mode: bool) -> float:
...
def dm_display_level(awareness: float, is_active_mode: bool) -> float:
  """Remap awareness [0,1] to the same 0-1 display fill used by the segment bar and dmoji rings (DAC parity)."""
  ...
  prompt_risk = float(np.clip(1.0 - _prompt_threshold(is_active_mode), 0.0, 1.0))
...
# Segment index where pair 1 (yellow) and pair 2 (orange) start lit; `n_lit = level * _N_SEGS` in _draw_horizontal_bar
_RING_NONE_MAX_N_LIT = 2.0
_RING_YELLOW_MAX_N_LIT = 4.0
...
def dm_display_ring_band(level: float) -> Literal["none", "yellow", "orange"]:
  """Ring bands track when each segment *pair* first gets lit, same as _draw_pair_horizontal (n_lit = level * 6)."""
  n = dm_n_lit_from_display_level(level)
  if n <= _RING_NONE_MAX_N_LIT + 1e-9:
    return "none"  # pair 0 (grey) only; segment indices 0–1
  if n <= _RING_YELLOW_MAX_N_LIT + 1e-9:
    return "yellow"  # any yellow (indices 2–3) lit, no orange (4–5) yet
  return "orange"  # first orange segment (index 4) has any fill
